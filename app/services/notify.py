...
class NotificationReferenceType(enum.StrEnum):
    INVITATION = "invitation"


@dataclasses.dataclass(frozen=True)
class NotificationReference:
    type_: NotificationReferenceType
    id: uuid.UUID

    @staticmethod
    def for_invitation(invitation: Invitation) -> "NotificationReference":
        return NotificationReference(type_=NotificationReferenceType.INVITATION, id=invitation.id)

    @staticmethod
    def from_reference(reference: str) -> "NotificationReference":
        try:
            prefix, type_, id_ = reference.split(":")
        ...
        return NotificationReference(type_=NotificationReferenceType(type_), id=uuid.UUID(id_))

    @property
    def reference(self) -> str:
        return f"db:{self.type_}:{self.id}"
...
class NotificationService:
    ...
    def send_access_grant_team_member_invited(
        ...
        return self._send_email(
            ...
            govuk_notify_reference=NotificationReference.for_invitation(invitation),
        )
