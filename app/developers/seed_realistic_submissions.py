"""Seed valid submissions for each grant recipient on a collection.
...
ordering. Skips grant recipients that already have a submission. Supports both TEST and
LIVE modes (matching the grant-recipient mode filter to the submission mode). A
configurable proportion of seeded submissions can additionally be driven through the
submit flow by emitting events directly (bypassing the SubmissionHelper to avoid firing
...
def seed_realistic_submissions(
    ...
    *,
    ...
    seed: int | None = None,
    mode: GrantRecipientModeEnum = GrantRecipientModeEnum.TEST,
) -> SeedReport:
    """Create up to `max_submissions` submissions for grant recipients matching `mode` on `collection_id`.
    ...
    grant_permissions = [RoleEnum.DATA_PROVIDER]
    if collection.requires_certification:
        grant_permissions.append(RoleEnum.CERTIFIER)
    ...
    for gr in grant_recipients:
        ...
        _attempt_submission_for_gr(
            collection, submission_user, gr, per_gr_attempts, should_submit, submission_mode, report
        )
...
def _attempt_submission_for_gr(
    ...
    for _ in range(per_gr_attempts):
        ...
        try:
            ...
            if should_submit:
                if collection.requires_certification:
                    SubmissionHelper(submission).mark_as_sent_for_certification(user)
                    SubmissionHelper(submission).certify(user)
...
def _maybe_submit_submission(
    submission: Any,
    user: User,
    gr: GrantRecipient,
    submit_proportion: int,
    report: SeedReport,
) -> None:
    """With probability `submit_proportion%`, emit the events that drive a submission to SUBMITTED.

    Bypasses `SubmissionHelper.{toggle_form_completed,mark_as_sent_for_certification,certify,submit}`
    so we don't fire notification emails or trip LIVE-only role checks. The populated submission has
    already been committed by the caller, so a failure here only rolls back the new events.
    """
    if random.randint(1, 100) > submit_proportion:
        return
    ...
    try:
        ...
        if helper.collection.requires_certification:
            _add_submission_event(
                submission, event_type=SubmissionEventType.SUBMISSION_SENT_FOR_CERTIFICATION, user=user
            )
            _add_submission_event(
                submission, event_type=SubmissionEventType.SUBMISSION_APPROVED_BY_CERTIFIER, user=user
            )
        _add_submission_event(submission, event_type=SubmissionEventType.SUBMISSION_SUBMITTED, user=user)
        db.session.commit()
        report.submitted += 1
    except Exception as exc:
        db.session.rollback()
...
def _draw_answer(question: Question, helper: SubmissionHelper, *, add_another_index: int | None = None) -> Any:
    match question.data_type:
        case QuestionDataType.TEXT_SINGLE_LINE:
            return TextSingleLineAnswer(_generate_single_line_text(question))
        case QuestionDataType.TEXT_MULTI_LINE:
            return TextMultiLineAnswer(_generate_multi_line_text(question))
...
def _generate_single_line_text(question: Question) -> str:
    if any(_managed_name(v) == ManagedExpressionsEnum.UK_POSTCODE for v in question.validations):
        return _fake.postcode()
    return _fake.sentence(nb_words=random.randint(4, 10)).rstrip(".")


def _generate_multi_line_text(question: Question) -> str:
    word_limit = question.word_limit
    max_words = word_limit if word_limit else 150
    paragraphs = []
    words_remaining = max_words
    for _ in range(random.randint(1, 3)):
        para_words = min(random.randint(10, 40), words_remaining)
        if para_words < 5:
            break
        paragraphs.append(_fake.paragraph(nb_sentences=max(1, para_words // 10)))
        words_remaining -= len(paragraphs[-1].split())
