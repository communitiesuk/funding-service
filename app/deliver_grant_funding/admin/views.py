...
class PlatformAdminReportingLifecycleView(FlaskAdminPlatformAdminGrantLifecycleManagerAccessibleMixin, BaseView):
    ...
    def set_up_grant_recipients(self, grant_id: UUID, collection_id: UUID) -> Any:
        ...
        if form.validate_on_submit():
            ...
            live_organisations = get_organisations(mode=OrganisationModeEnum.LIVE)
            live_organisation_external_ids = {
                org.external_id
                for org in live_organisations
                if str(org.id) in form.recipients.data  # type: ignore[operator]
            }

            test_organisations = get_organisations(mode=OrganisationModeEnum.TEST)
            filtered_test_organisations = list(
                filter(lambda org: org.external_id in live_organisation_external_ids, test_organisations)
            )
            test_organisation_ids = [org.id for org in filtered_test_organisations]
            create_grant_recipients(
            ...
            for test_organisation in filtered_test_organisations:
