...
def register_admin_views(flask_admin: Admin, db: SQLAlchemy) -> None:
    ...
    flask_admin.add_view(PlatformAdminUserRoleView(db.session))
    flask_admin.add_view(PlatformAdminOrganisationView(db.session))
    flask_admin.add_view(PlatformAdminCollectionView(db.session))
