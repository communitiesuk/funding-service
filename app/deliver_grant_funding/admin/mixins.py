...
def _is_sso() -> bool:
    ...
    if session.get("auth") != AuthMethodEnum.SSO:
        return False
    return user.email.endswith(current_app.config["INTERNAL_DOMAINS"])


class FlaskAdminPlatformAdminAccessibleMixin:
    def is_accessible(self) -> bool:
        return AuthorisationHelper.is_platform_admin(get_current_user()) and _is_sso()
...
class FlaskAdminPlatformAdminGrantLifecycleManagerAccessibleMixin:
    def is_accessible(self) -> bool:
        return (
            AuthorisationHelper.has_platform_admin_role(RoleEnum.GRANT_LIFECYCLE_MANAGER, get_current_user())
            and _is_sso()
        )
...
class FlaskAdminPlatformAdminDataAnalystAccessibleMixin:
    def is_accessible(self) -> bool:
        return AuthorisationHelper.has_platform_admin_role(RoleEnum.DATA_ANALYST, get_current_user()) and _is_sso()
...
class FlaskAdminPlatformMemberAccessibleMixin:
    def is_accessible(self) -> bool:
        return AuthorisationHelper.is_platform_member(get_current_user()) and _is_sso()
