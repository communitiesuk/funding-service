...
def upload_data_set(grant_id: UUID, collection_type: CollectionType, collection_id: UUID) -> ResponseReturnValue:
    ...
    if form.validate_on_submit():
        ...
        s3_key = build_data_set_upload_s3_key(grant_id=grant_id, report_id=collection_id, data_source_id=data_source_id)
        file.stream.seek(0)
        ...
        if form.data_source_type.data != DataSourceType.STATIC:
            grant_recipients = interfaces.grant_recipients.get_grant_recipients(
                collection.grant, with_organisations=True
            )
...
def view_data_source(
    ...
    if data_source.type in [DataSourceType.GRANT_RECIPIENT, DataSourceType.PROJECT_LEVEL]:
        all_grant_recipients = interfaces.grant_recipients.get_grant_recipients(
            collection.grant, with_organisations=True
        )
