...
class ManagedExpression(BaseModel, ABC):
    # Defining this as a ClassVar allows direct access from the class and excludes it from pydantic instance
    # Used in the underproved+overcooked demo that lets an expression definition "magically" inject itself into
    # the "add condition/validation" pages.
    # This is maybe one of those fun things to show, but might be too "clever" for its own good (ie hard to trace,
    # understand, extend, vary, etc).
