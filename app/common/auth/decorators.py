...
from app.constants import SESSION_SIGNING_UP_FOR_COLLECTION_ID
...
def access_grant_funding_login_required[**P](
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> ResponseReturnValue:
        ...
        if collection_id := session.get(SESSION_SIGNING_UP_FOR_COLLECTION_ID):
...
def is_signing_up[**P](
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> ResponseReturnValue:
        ...
        if user.is_authenticated and session.get(SESSION_SIGNING_UP_FOR_COLLECTION_ID) == collection.id:
            return func(*args, **kwargs)
