...
Revision ID: 043_uploaded_datasource
...
revision = "043_uploaded_datasource"
...
def upgrade() -> None:
    ...
    op.create_table(
        "data_source_grant_recipient_item",
        ...
        sa.Column("grant_recipient_id", sa.Uuid(), nullable=False),
        sa.Column("data", postgresql.JSONB(astext_type=sa.Text()), nullable=False),
        sa.ForeignKeyConstraint(
            ["data_source_id"],
            ["data_source.id"],
            name=op.f("fk_data_source_grant_recipient_item_data_source_id_data_source"),
        ),
        sa.ForeignKeyConstraint(
            ["grant_recipient_id"],
            ["grant_recipient.id"],
            name=op.f("fk_data_source_grant_recipient_item_grant_recipient_id_grant_recipient"),
        ),
        sa.PrimaryKeyConstraint("id", name=op.f("pk_data_source_grant_recipient_item")),
        sa.UniqueConstraint("data_source_id", "grant_recipient_id", name="uq_data_source_grant_recipient"),
    )
    with op.batch_alter_table("data_source_grant_recipient_item", schema=None) as batch_op:
        batch_op.create_index("ix_data_source_grant_recipient_item_data_source_id", ["data_source_id"], unique=False)
        batch_op.create_index(
            "ix_data_source_grant_recipient_item_grant_recipient_id", ["grant_recipient_id"], unique=False
        )
    ...
    with op.batch_alter_table("data_source", schema=None) as batch_op:
        ...
        batch_op.create_check_constraint(
            "ck_data_source_non_custom_requires_schema",
            "type = 'CUSTOM' OR schema IS NOT NULL",
        )
        batch_op.create_check_constraint(
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("data_source", schema=None) as batch_op:
        ...
        batch_op.drop_constraint("ck_data_source_non_custom_requires_name_grant_and_collection", type_="check")
        batch_op.drop_constraint("ck_data_source_non_custom_requires_schema", type_="check")
    ...
    with op.batch_alter_table("data_source_grant_recipient_item", schema=None) as batch_op:
        batch_op.drop_index("ix_data_source_grant_recipient_item_grant_recipient_id")
        batch_op.drop_index("ix_data_source_grant_recipient_item_data_source_id")

    op.drop_table("data_source_grant_recipient_item")
