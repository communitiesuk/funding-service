...
class ReferenceFlash(BaseModel):
    component_id: UUID
    component_text: InterpolationStatement
    is_group: bool
    reference_type: Literal["Question", "Group", "Validation", "Condition"]
...
class DataSourceHasReferencesException(Exception, FlashableException):
    def __init__(
        ...
        references: list[ReferenceFlash],
    ):
        super().__init__(message)
        self.message = message
    ...
    def as_flash_context(self) -> dict[str, Any]:
        return {
            ...
            "references": sorted(
                [ref.model_dump() for ref in self.references], key=lambda r: (r["component_text"], r["reference_type"])
            ),
        }
...
def raise_if_data_source_has_references(data_source: DataSource) -> Never | None:
    ...
    reference_type: Literal["Question", "Group", "Validation", "Condition"]
    seen_keys: set[tuple[UUID, str]] = set()
    references_for_flash: list[ReferenceFlash] = []
    for ref in references:
        ...
        references_for_flash.append(
            ReferenceFlash(
                component_id=ref.component.id,
                component_text=ref.component.text,
