...
def copy_collection(collection: Collection, *, name: str, user: User, grant: Grant) -> Collection:  # noqa: C901
    ...
    for data_source_data in remapped["data_sources"]:
        ...
        ds = DataSource(**data_source_data)
        db.session.add(ds)

        if data_source_data["type"] == DataSourceType.GRANT_RECIPIENT:
            upload_header_only_data_set_files(ds)

            for grant_recipient in get_grant_recipients(grant, with_organisations=True):
                db.session.add(
                    DataSourceOrganisationItem(
...
@flush_and_rollback_on_exceptions
def delete_all_collection_preview_submissions(collection: Collection) -> None:
    submission_ids = db.session.scalars(
        select(Submission.id).where(
            Submission.collection_id == collection.id,
