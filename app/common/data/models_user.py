...
class User(BaseModel):
    ...
    # TODO make this nullable=False once all users have a last_login_utc set
    last_login_utc: Mapped[datetime | None] = mapped_column(nullable=True)
...
class MagicLink(BaseModel):
    ...
    @hybrid_property
    def is_usable(self):
        # TODO find out why we remove tzinfo
        return (self.claimed_at_utc is None) and (self.expires_at_utc > func.now())
...
class Invitation(BaseModel):
    ...
    def is_usable(self):
        # TODO find out why we remove tzinfo
        return self.claimed_at_utc is None and self.expires_at_utc > func.now(utc).replace(tzinfo=None)
