...
class CreateOrganisationSession(SignUpSession):
    ...
    def first_incomplete_page(self) -> CreateOrganisationPage:
        ...
        """
        return next(page for page in self._pages if not self._is_answered(page))
    ...
    def _pages(self) -> list[CreateOrganisationPage]:
        """The pages this journey visits, in order; navigation, completeness and access are all derived from it."""
    ...
    def _page_after(self, page: CreateOrganisationPage) -> CreateOrganisationPage | None:
        ...
        return next((candidate for candidate in self._pages if journey.index(candidate) > journey.index(page)), None)
    ...
    def validate_for_page(
        ...
        *,
        ...
        if self.organisation_type is None or not (self.name and self.external_id):
            self.name = None
