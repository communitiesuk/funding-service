...
def test_create_view_edit_grant_success(page: Page, domain: str):
    ...
    all_grants_page = new_grant_page.submit_new_grant_form()
    ...
    new_grant_page.submit_new_grant_form()
    ...
    # Grant Settings
    grant_settings_page = grant_dashboard_page.go_to_settings()
    expect(page.get_by_role("heading", name=f"{new_grant_name} Settings")).to_be_visible()

    # Change grant name
    change_grant_name_page = grant_settings_page.go_to_change_grant_name()
    ...
    change_grant_name_page.submit_change_grant_name_form()
