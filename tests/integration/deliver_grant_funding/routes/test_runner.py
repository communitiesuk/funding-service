...
class TestGroupValidation:
    ...
    def test_post_same_page_group_passing_validation_redirects(
        ...
        assert submission.data.get(str(capital.id)) == {"value": 400}
        assert submission.data.get(str(revenue.id)) == {"value": 600}
    ...
    def test_post_same_page_group_failing_validation_shows_on_page_summary(
        ...
        db_session.refresh(submission)
        assert submission.data.get(str(capital.id)) is None
        assert submission.data.get(str(revenue.id)) is None
    ...
    def test_post_submission_tasklist_blocks_when_group_validation_fails(
        ...
        submission = factories.submission.create(
            ...
            created_by=client.user,
            data={str(capital.id): {"value": 200}, str(revenue.id): {"value": 300}},
        )
        factories.submission_event.create(
            created_by=client.user,
            submission=submission,
