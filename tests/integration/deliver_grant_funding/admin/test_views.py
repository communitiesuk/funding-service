...
class TestReportingLifecycleTasklist:
    ...
    def test_shows_all_tasklists(self, authenticated_platform_admin_client, factories, db_session):
        ...
        assert task_title is not None
        assert task_title.get_text(strip=True) == "Send emails to grant recipients"
    ...
    def test_send_emails_task_do_once_when_open(self, authenticated_platform_admin_client, factories, db_session):
        ...
        assert task_title.get_text(strip=True) == "Send emails to grant recipients"
        assert (
            f"/deliver/admin/reporting-lifecycle/{grant.id}/{collection.id}/send-emails-to-recipients"
            in task_title.get("href")
        )
...
class TestSendEmailsToRecipients:
    ...
    def test_send_emails_to_recipients_page_content(self, authenticated_platform_admin_client, factories, db_session):
        ...
        assert "Send emails to grant recipients" in h1.get_text(strip=True)
        ...
        assert (
            f"/deliver/admin/reporting-lifecycle/{grant.id}/{collection.id}/send-emails-to-recipients/download-csv"
...
class TestMakeReportLive:
    ...
    def test_post_fails_when_grant_recipients_have_no_users(
        ...
        soup = BeautifulSoup(response.data, "html.parser")
        assert page_has_error(soup, "All grant recipients must have at least one user set up before opening a report")
