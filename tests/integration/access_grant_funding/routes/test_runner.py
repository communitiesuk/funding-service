...
class TestAskAQuestion:
    ...
    @pytest.mark.parametrize("data_to_submit_for_number_question", [(None), (""), ("words")])
    def test_post_ask_a_question_with_error_on_number_format_with_dependent_condition(
        self, authenticated_grant_recipient_data_provider_client, factories, data_to_submit_for_number_question
    ):
        ...
        assert response.status_code == 200
        soup = BeautifulSoup(response.data, "html.parser")
        assert page_has_error(soup, "The answer must be a whole number, like 100")
