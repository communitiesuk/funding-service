...
from app.common.expressions.managed import GreaterThan, IsYes, Specifically
...
class TestExpressionModel:
    def test_condition_summary_names_the_answer_number_and_question(self, factories):
        ...
        assert question.conditions[0].condition_summary == "Shown if “yes” to 2 (Favourite colour)"

    def test_condition_summary_uses_the_number_of_a_nested_subjects_group(self, factories):
        user = factories.user.create()
        ...
        assert question.conditions[0].condition_summary == "Shown if “yes” to 1 (Favourite colour)"

    def test_condition_summary_for_custom_conditions_uses_their_own_message(self, factories):
        user = factories.user.create()
        question = factories.question.create()
        add_component_condition(
            component=question,
        ...
        assert question.conditions[0].condition_summary == "Shown if the total is over budget"

    def test_condition_summary_for_custom_conditions_without_a_message_uses_their_name(self, factories):
        user = factories.user.create()
        question = factories.question.create()
        add_component_condition(
            component=question,
            user=user,
            evaluatable_expression=CustomExpression(
                expression_name="The total is over budget",
                custom_expression=EvaluationStatement("1 == 1"),
            ),
        )
...
class TestFormModel:
    ...
    def test_display_section_index_continues_from_earlier_sections(self, factories):
        collection = factories.collection.create()
        section_one = factories.form.create(collection=collection, order=0)
        section_two = factories.form.create(collection=collection, order=1)
        ...
        assert section_one.display_section_index == 1
        assert section_two.display_section_index == 3
        assert section_three.display_section_index == 4
...
class TestDataSourceModel:
    ...
    def test_has_missing_referenced_data_for_organisation_ignores_unreferenced_null_columns(
        ...
        factories.question.create(
            form__collection=report,
            hint=InterpolationStatement(
                "You were allocated " + ExpressionReference.from_data_source_column(data_source, "c_just_text").wrapped
            ),
        )
