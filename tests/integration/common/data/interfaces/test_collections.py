...
class TestGetCollection:
    ...
    def test_get_collection_with_full_schema_data_source_references_no_n_plus_one(
        ...
        for i in range(2):
            ...
            factories.question.create(
                ...
                hint=InterpolationStatement(
                    "You were allocated "
                    + ExpressionReference.from_data_source_column(other_data_source, "c_allocation").wrapped
                ),
            )
        db_session.commit()
...
class TestUpdateCollection:
    ...
    def test_update_collection_enable_multiple_submissions(self, db_session, factories):
        ...
        updated = update_collection(
            ...
            submission_guidance="  Some guidance  ",
        )
        ...
        assert updated.submission_guidance == "Some guidance"
    ...
    def test_update_collection_enable_multiple_submissions_stores_blank_guidance_as_none(self, db_session, factories):
        collection = factories.collection.create()
        question = factories.question.create(form__collection=collection, data_type=QuestionDataType.TEXT_SINGLE_LINE)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            {"submission_guidance": "Some guidance"},
            {"allow_multiple_submissions": True, "multiple_submissions_are_managed_by_service": False},
            {"allow_multiple_submissions": False, "submission_name_question_id": None, "submission_guidance": None},
        ],
    )
    def test_update_collection_multiple_submission_settings_raise_unless_all_provided(
        self, db_session, factories, partial_settings
    ):
        collection = factories.collection.create()

        with pytest.raises(ValueError, match="must all be provided together"):
            update_collection(collection, **partial_settings)

    def test_update_collection_enable_multiple_submissions_without_question_raises(self, db_session, factories):
        collection = factories.collection.create()

        with pytest.raises(ValueError, match="submission_name_question_id must be provided"):
            update_collection(
                collection,
                allow_multiple_submissions=True,
                multiple_submissions_are_managed_by_service=False,
    ...
    def test_update_collection_disable_multiple_submissions_clears_settings(self, db_session, factories):
        ...
        collection.submission_guidance = "Existing guidance"
        db_session.commit()
    ...
    @pytest.mark.parametrize(
        "setting", ["multiple_submissions_are_managed_by_service", "submission_name_question_id", "submission_guidance"]
    )
    def test_update_collection_disable_multiple_submissions_raises_for_enabled_values(
        self, db_session, factories, setting
    ):
        collection = factories.collection.create()
        question = factories.question.create(form__collection=collection, data_type=QuestionDataType.TEXT_SINGLE_LINE)
        enabled_values = {
        ...
        settings = {
            ...
            "submission_name_question_id": None,
            "submission_guidance": None,
        } | {setting: enabled_values[setting]}

        with pytest.raises(ValueError, match="allow_multiple_submissions cannot be set to False"):
            update_collection(collection, **settings)
    ...
    def test_update_collection_submission_name_question_raises_for_nonexistent_id(self, db_session, factories):
        ...
        with pytest.raises(ValueError, match="submission_name_question_id must be a question ID"):
            update_collection(
                collection,
                allow_multiple_submissions=True,
                multiple_submissions_are_managed_by_service=False,
                submission_name_question_id=uuid.uuid4(),
                submission_guidance=None,
            )
    ...
    def test_update_collection_submission_name_question_raises_for_group(self, db_session, factories):
        ...
        with pytest.raises(ValueError, match="submission_name_question_id must be a question ID"):
            update_collection(
                ...
                allow_multiple_submissions=True,
                multiple_submissions_are_managed_by_service=False,
                submission_name_question_id=group.id,
                submission_guidance=None,
            )
    ...
    def test_update_collection_change_submission_name_question_raises_when_live_submissions_exist(
        ...
        with pytest.raises(ValueError, match="Cannot change the submission name question"):
            update_collection(
                ...
                allow_multiple_submissions=True,
                multiple_submissions_are_managed_by_service=False,
                submission_name_question_id=other_question.id,
                submission_guidance=None,
            )
    ...
    def test_update_reminder_email_business_days(self, db_session, factories):
        collection = factories.collection.create(reminder_email_business_days_before_closing=5)

        updated = update_collection(collection, reminder_email_business_days_before_closing=3)

        assert updated.reminder_email_business_days_before_closing == 3

        from_db = db_session.get(Collection, collection.id)
        assert from_db.reminder_email_business_days_before_closing == 3

    def test_update_reminder_email_business_days_not_provided_leaves_unchanged(self, db_session, factories):
        collection = factories.collection.create(reminder_email_business_days_before_closing=7)
...
class TestUpdateGroup:
    ...
    def test_update_group_with_add_another_guidance_reference(self, db_session, factories):
        form = factories.form.create()
...
class TestDeleteCollectionSubmissions:
    def test_delete_preview_collection_submission_created_by_user(self, db_session, factories):
        collection = factories.collection.create(
        ...
        assert len(users_submissions_from_db) == 2
        ...
        # Check that only the specified user's preview submission & its associated SubmissionEvents were
        # deleted, and no live submission was deleted
        assert len(preview_submissions_from_db) == 2
        assert len(live_submissions_from_db) == 3
...
class TestReferenceValidation:
    ...
    def test_components_in_valid_add_another_combination(self, factories, subtests):
        q1 = factories.question.create(add_another=True)
        g1 = factories.group.create(form=q1.form, add_another=True)
        ...
        with subtests.test("add_another referencing previous non-add_another"):
            assert (
        ...
        with subtests.test("non-add_another referencing previous add_another"):
            assert (
                components_in_valid_add_another_combination(
                    q2,
                    [
        ...
        with subtests.test("add_another in group referencing previous add_another from outside group"):
            assert components_in_valid_add_another_combination(g1_q1, [q1, None]) is False
        with subtests.test("add_another in group referencing previous add_another in same group"):
            assert components_in_valid_add_another_combination(g1_q1, [g1_q2]) is True
        with subtests.test("add_another in group referencing previous add_another in same group, with None"):
            assert components_in_valid_add_another_combination(g1_q1, [g1_q2, None]) is True
        with subtests.test("non-add_another referencing previous add_anothers in same group as each other"):
            assert components_in_valid_add_another_combination(q3, [g1_q1, g1_q2]) is False
        with subtests.test("non-add_another referencing previous add_anothers in same group as each other, with None"):
            assert components_in_valid_add_another_combination(q3, [g1_q1, g1_q2, None]) is False

        with subtests.test("non-add_another referencing previous non-add_another"):
            assert components_in_valid_add_another_combination(q2, [q3]) is True

        with subtests.test("add_another referencing previous add_another from different group"):
            assert components_in_valid_add_another_combination(g1_q2, [g2_q1]) is False
    ...
    def test_creates_column_references_for_both_between_bounds(self, db_session, factories):
        ...
        expression = Expression.from_evaluatable_expression(
            Between(
                subject_reference=ExpressionReference.from_question(question),
                minimum_value=None,
                minimum_expression=ExpressionReference.from_data_source_column(data_source, "c_min"),
                maximum_value=None,
    ...
    def test_rejects_column_reference_to_missing_column(self, db_session, factories):
        ...
        expression = Expression.from_evaluatable_expression(
            GreaterThan(
                subject_reference=ExpressionReference.from_question(question),
                minimum_value=None,
                minimum_expression=ExpressionReference.from_data_source_column(data_source, "c_missing"),
            ),
            ExpressionType.VALIDATION,
            user,
        )
...
class TestValidateReference:
    ...
    def test_validate_reference_ordering_enforced_in_non_expression(self, factories):
        q1 = factories.question.create()
        g = factories.group.create(form=q1.form, add_another=True)
        q2 = factories.question.create(form=q1.form)
        with pytest.raises(DependencyOrderException):
    ...
    def test_validate_reference_valid_reference_to_self(self, factories):
        form = factories.form.create()
        q1, q2, q3 = factories.question.create_batch(3, form=form, data_type=QuestionDataType.NUMBER)
    ...
    def test_custom_expression_bad_order_validation(self, factories):
        ...
        assert f"You cannot use {q3.name} because it comes after this question" in e.value.form_error_message
    ...
    def test_invalid_add_another_reference(self, factories):
        form = factories.form.create()
        group = factories.group.create(form=form, add_another=True)
        g1_q1 = factories.question.create(form=form, parent=group, data_type=QuestionDataType.NUMBER)
        g1_q2 = factories.question.create(form=form, parent=group, data_type=QuestionDataType.NUMBER)
        q1 = factories.question.create(form=form, data_type=QuestionDataType.NUMBER)
        q2 = factories.question.create(form=form, data_type=QuestionDataType.NUMBER)
    ...
    def test_valid_add_another_reference(self, factories):
        form = factories.form.create()
        q1 = factories.question.create(form=form, data_type=QuestionDataType.NUMBER)
        group = factories.group.create(form=form, add_another=True)
        g1_q1 = factories.question.create(form=form, parent=group, data_type=QuestionDataType.NUMBER)
...
class TestGetCollectionsByStatusExcludingDraftGrants:
    ...
    def test_returns_collections_matching_multiple_statuses(self, db_session, factories):
        ...
        assert open_collection.id in result_ids
