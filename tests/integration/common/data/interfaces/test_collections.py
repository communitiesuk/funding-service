...
def test_add_submission_event(db_session, factories):
    ...
    add_submission_event(
        submission=submission, user=user, event_type=SubmissionEventType.FORM_RUNNER_FORM_COMPLETED, target_key=form.id
    )
    ...
    assert from_db.events[0].target_key == form.id
    ...
    assert from_db.events[1].target_key is submission.id
...
def test_clear_events_from_submission(db_session, factories):
    ...
    clear_submission_events(
        submission=submission, event_type=SubmissionEventType.FORM_RUNNER_FORM_COMPLETED, target_key=form_one.id
    )
    assert len(submission.events) == 1
    assert submission.events[0].target_key == form_two.id
